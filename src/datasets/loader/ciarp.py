...
def load_ciarp(config, path=None):
    ...
    X, meta = hd.load(config['data.dataset'], Path(
        data_dir), version=config['data.version'])

    if config['data.split']:
        split_dir = os.path.join(path, 'splits', config['data.split'])
        def split_file(split): return os.path.join(split_dir, f"{split}.txt")

        x_train, y_train = load_from_split(
            config['data.dataset'], config['data.version'], data_dir, split_file('train'))
        x_test, y_test = load_from_split(
            config['data.dataset'], config['data.version'], data_dir, split_file('test'))
        x_val, y_val = load_from_split(
            config['data.dataset'], config['data.version'], data_dir, split_file('val'))
    else:
        x, y = X, meta['y']
        ...
        x_train, x_val, y_train, y_val = train_test_split_balanced(
            x_train, y_train, train_size=0.8, n_train_per_class=n_train_per_class, test_size=0.2)
